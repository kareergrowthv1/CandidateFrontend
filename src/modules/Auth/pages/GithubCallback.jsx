...
export default function GithubCallback() {
    ...
    useEffect(() => {
        ...
        const handleGithubLogin = async () => {
            console.log('[GithubCallback] Search:', window.location.search);
            ...
            if (status === 'success') {
                try {
                    // 1. Save tokens to localStorage (ensures cross-port auth works)
                    if (urlAccessToken) {
                    ...
                    console.error('GitHub sign-in verification failed:', err);
                    showToast('Failed to complete sign-in. Please try again.', 'error');
                    navigate('/login');
                }
            } else {
                const message = params.get('message') || 'GitHub login was cancelled or failed.';
